...
class Frame:
	...
	def draw_bodies(self, destination_image, dest_camera = K4A_CALIBRATION_TYPE_DEPTH, only_segments=False, movement_detected=False, impact_detected=False, joint_head_name=None, impact_position=None, face_region=None):
		num_bodies = self.get_num_bodies()

		for body_id in range(num_bodies):
			destination_image = self.draw_body2d(destination_image, body_id, dest_camera, only_segments, movement_detected, impact_detected, joint_head_name, impact_position, face_region)

		return destination_image

	def draw_body2d(self, destination_image, bodyIdx = 0, dest_camera = K4A_CALIBRATION_TYPE_DEPTH, only_segments = False, movement_detected = False, impact_detected = False, joint_head_name=None, impact_position=None, face_region=None):
		return self.get_body2d(bodyIdx, dest_camera).draw(destination_image, only_segments, movement_detected, impact_detected, joint_head_name, impact_position, face_region)
